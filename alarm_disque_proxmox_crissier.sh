...
# watch -n 1 './alarm_disque_proxmox_crissier.sh'
# dd if=/dev/zero of=bigfile1 bs=1M count=1000

#cp /keybase/private/zuzu59/secrets_alarm_telegram_zf.sh .
#crontab -e
#* * * * * /root/dev/alarm_telegram/alarm_disque_proxmox_crissier.sh > /dev/null 2>&1 || true
...
ZVAL=$(/bin/df -k |/bin/grep '/mnt/pve/ext-1' |/usr/bin/awk '{print $4}')
#ZCONSIGN_ON=550000000
ZCONSIGN_ON=80000000
ZHYSTERESE=100000
...
#    $(/usr/bin/dirname $0)/send_alarm_telegram.sh 'NOC-tst, alarme manque place disque, '$ZVAL'kB' zf
    $(/usr/bin/dirname $0)/send_alarm_telegram.sh 'ext-1 proxmox Crissier, alarme manque place disque, ne reste plus que '$ZVAL' kB' ctrl_alrm_zf
#    $(/usr/bin/dirname $0)/send_alarm_telegram.sh 'NOC-tst, alarme manque place disque, ne reste plus que '$ZVAL'kB' ctrl_alrm_noc
  fi
else
...
#      $(/usr/bin/dirname $0)/send_alarm_telegram.sh 'NOC-tst, retour à la normale, '$ZVAL'kB' zf
      $(/usr/bin/dirname $0)/send_alarm_telegram.sh 'ext-1 proxmox Crissier, retour à la normale, il reste '$ZVAL' kB' ctrl_alrm_zf
